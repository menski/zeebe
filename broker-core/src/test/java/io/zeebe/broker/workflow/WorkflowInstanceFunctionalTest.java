...
public class WorkflowInstanceFunctionalTest {
  ...
  public void shouldOccureEndEvent() {
    ...
    // then
    final SubscribedRecord event =
        testClient.receiveEvents().withIntent(WorkflowInstanceIntent.END_EVENT_OCCURRED).getFirst();
  ...
  public void shouldActivateServiceTask() {
    ...
    // then
    final SubscribedRecord event =
        testClient.receiveEvents().withIntent(WorkflowInstanceIntent.ACTIVITY_ACTIVATED).getFirst();
  ...
  public void shouldCreateTaskWhenServiceTaskIsActivated() {
    ...
    // then
    final SubscribedRecord event =
        testClient.receiveCommands().ofTypeJob().withIntent(JobIntent.CREATE).getFirst();
  ...
  public void shouldCompleteServiceTaskWhenTaskIsCompleted() {
    ...
    final SubscribedRecord activityCompletedEvent =
        testClient.receiveEvents().withIntent(WorkflowInstanceIntent.ACTIVITY_COMPLETED).getFirst();
  ...
  public void shouldSpitOnExclusiveGateway() {
    ...
    SubscribedRecord endEvent =
        receiveWorkflowInstanceEvent(workflowInstance1, WorkflowInstanceIntent.END_EVENT_OCCURRED);
    assertThat(endEvent.value()).containsEntry(PROP_WORKFLOW_ACTIVITY_ID, "a");

    endEvent =
        receiveWorkflowInstanceEvent(workflowInstance2, WorkflowInstanceIntent.END_EVENT_OCCURRED);
    assertThat(endEvent.value()).containsEntry(PROP_WORKFLOW_ACTIVITY_ID, "b");

    endEvent =
        receiveWorkflowInstanceEvent(workflowInstance3, WorkflowInstanceIntent.END_EVENT_OCCURRED);
