...
public class MockStreamProcessorController<T extends UnpackedObject> extends ExternalResource {
  ...
  // EventType defaultEventType, long initialPosition)
  //    {
  ...
  //    }
  //
  //    public MockStreamProcessorController(Class<T> eventClass, EventType eventType, long
  // initialPosition)
  //    {
  //        this(eventClass, (t) ->
  ...
  //        doAnswer(invocation ->
  //        {
  //            final WrittenEvent<T> lastWrittenEvent = new WrittenEvent<>(lastEventKey,
  // lastEventValue, lastEventMetadata);
  ...
  //    }
  //
  //    public void initStreamProcessor(StreamProcessor streamProcessor, StreamProcessorContext
  // context)
  //    {
  ...
  //    }
  //
  //    public void processEvent(long key, Consumer<T> eventSetter, Consumer<BrokerEventMetadata>
  // metadataSetter)
  //    {
  //        Objects.requireNonNull(streamProcessor, "No stream processor set. Call
  // 'initStreamProcessor()' in setup method.");
  //
  ...
  //
  //    public LoggedEvent buildLoggedEvent(long key, Consumer<T> eventSetter,
  // Consumer<BrokerEventMetadata> metadataSetter)
  //    {
  //
  ...
  //
  //        final T event = newEventInstance();
  //        final DirectBuffer buf = populateAndWrite(event,
  // defaultEventSetter.andThen(eventSetter));
  //
  //        doAnswer(invocation ->
  //        {
  ...
  //
  //        final BrokerEventMetadata metaData = new BrokerEventMetadata();
  //        final DirectBuffer metaDataBuf = populateAndWrite(metaData,
  // defaultMetadataSetter.andThen(metadataSetter));
  //        doAnswer(invocation ->
  //        {
  //            final BufferReader arg = (BufferReader) invocation.getArguments()[0];
  ...
  //    }
  //
  //    protected <S extends BufferWriter> DirectBuffer populateAndWrite(S writer, Consumer<S>
  // setter)
  //    {
